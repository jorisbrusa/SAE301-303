// Load data from CSV and execute visualizations
d3.dsv(',', '../CSV/data.csv')
    .then(cleanedData => {
        console.log("Données chargées :", cleanedData);
        console.log("Colonnes disponibles :", Object.keys(cleanedData[0]));

        // Filter data for the specific project "4SYSLEG"
        const projectData = cleanedData.filter(row => row['Nom du projet']?.trim() === '4SYSLEG');
        console.log("Données filtrées pour 4SYSLEG :", projectData);

        if (projectData.length === 0) {
            console.error("Aucune donnée trouvée pour le projet 4SYSLEG. Vérifiez le nom du projet.");
            return;
        }
        ...
        new Chart(document.getElementById('chart1_1').getContext('2d'), {
            type: 'pie',
            data: {
        ...
        new Chart(document.getElementById('chart2_1').getContext('2d'), {
            type: 'bar',
            data: {
                labels: methodLabels,
                datasets: datasets,
        ...
        new Chart(document.getElementById('chart3_1').getContext('2d'), {
            type: 'doughnut',
            data: {
